  ...
  return (
    ...
    >
      ...
      <div className="mt-2 space-y-1.5 text-sm">
        ...
        <div
          data-score-row
          className="flex justify-between border-t border-white/10 pt-1.5 text-slate-400"
        >
          <span>Total before minimum</span>
          <span className="font-medium">{breakdown.rawScore}</span>
        </div>
        {breakdown.wasFloored && (
          <p data-score-row className="text-xs text-amber-300">
            Final score is 0 — scores cannot go below zero.
          </p>
        )}
        <div
          data-score-row
          className="flex justify-between border-t border-white/10 pt-1.5 font-semibold text-slate-100"
        >
