...
export function WordSearchGrid() {
    ...
    const handlePointerMove = useCallback((e) => {
        if (disabled || !gridRef.current?.hasPointerCapture(e.pointerId))
            return;
    ...
    return (<div className="flex h-full min-h-0 w-full touch-none select-none items-center justify-center">
      <div className="aspect-square h-full max-h-full w-full max-w-full rounded-lg border border-[#4a4238] bg-[#1f1c18] p-1.5">
        <div ref={gridRef} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} className="grid h-full w-full gap-0.5 sm:gap-1" style={{
