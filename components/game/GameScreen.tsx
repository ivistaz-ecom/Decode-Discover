...
export function GameScreen() {
  ...
  useEffect(() => {
    if (phase !== "playing" || submitting || celebratingVictory) return;
    if (placedWords.length === 0) return;
    if (foundWordIds.length < placedWords.length) return;
  ...
  if (authLoading || gameLoading) {
    return (
      <AppShell className="flex min-h-screen flex-col items-center justify-center gap-4 px-4">
        <LoadingSpinner label="Loading game..." />
        {loadingTimedOut && (
          <div className="max-w-md rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-center text-sm text-amber-100">
            ...
            <button
              ...
              className={`mt-3 ${glassPrimaryButtonClass}`}
            >
              Refresh
            </button>
          </div>
  ...
  return (
    <AppShell className="h-dvh overflow-hidden">
      ...
      {error && (
        <AnimateIn className="shrink-0 px-3 pt-1 sm:px-4">
