...
function getIntroSecondsRemaining(introStartedAt: number | null): number {
  if (!introStartedAt) {
...
export function FullscreenImageScreen({
  ...
  const [secondsLeft, setSecondsLeft] = useState(() =>
    getIntroSecondsRemaining(introStartedAt)
  );
  ...
  useGSAP(
    () => {
      if (!imageRef.current || !footerRef.current) return;
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-40 flex flex-col bg-black">
      ...
      <div
        ref={footerRef}
        className="flex items-center justify-between gap-4 border-t border-white/10 bg-black/80 px-4 py-3 sm:px-6"
      >
